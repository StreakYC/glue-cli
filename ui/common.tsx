...
export const AuthTriggerList = ({ triggers }: { triggers: TriggerDTO[] }) => {
  return (
    <Box paddingLeft={4} display="flex" flexDirection="column" gap={0}>
      ...
      {triggers.toSorted((a, b) => a.label.localeCompare(b.label)).filter((t) => !!t.accountSetupUrl).map((t) => (
        <Text key={t.id}>
...
export const SetupTriggerList = ({ triggers }: { triggers: TriggerDTO[] }) => {
  return (
    <Box paddingLeft={4} display="flex" flexDirection="column" gap={0}>
      {triggers.toSorted((a, b) => a.label.localeCompare(b.label)).filter((t) => !!t.routingId).map((t) => (
        <Text key={t.id}>
